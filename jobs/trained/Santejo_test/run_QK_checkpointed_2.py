...
#---- ENCODING SELECTION ----
#mirrors the same pattern as run_PQK_checkpointed.py, covering all 6
#encodings tested in the paper's Table 1: RotX, 3D, 3D-CNOT, ZZFeatureMap,
#IQP, Trotterized. Change encoding_key (and full_ent, for the 'xyz' case)
#to select which row you're reproducing.
full_ent = False  # set True for the "3D-CNOT" variant of xyz_encoded
...
#encoding half: data-dependent, built from the same Circuits.py library PQK
#uses - this is the actual "which encoding does this row test" choice
encoding_circuit = encoding_builders[encoding_key](NUM_QBIT)

#trainable half: one RY per qubit (6 parameters total) - kept as the
#original run_QK.py's simple ansatz for now (see conversation note: this is
#deliberately less expressive than PQK's 18-parameter zy_decomposition;
#swap this block for Circuits.zy_decomposition(...) if you want QK and PQK
#to be trained with an equally expressive trainable layer)
training_params = ParameterVector('theta_par', NUM_QBIT)
trainable_circuit = QuantumCircuit(NUM_QBIT)
for i, param in enumerate(training_params):
    trainable_circuit.ry(param, trainable_circuit.qubits[i])

#encoding first, then trainable - matches run_PQK_checkpointed.py's
#ordering, and is the semantically correct order for circuits like
#ZZFeatureMap/IQP/Trotter which are designed to act on the |0...0> state,
#not to be preceded by extra rotations
...
run_id = f'run_QK_{encoding_key}_ent{full_ent}_seed{seed}'
